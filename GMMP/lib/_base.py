...
class BaseMixture(DensityMixin, BaseEstimator, metaclass=ABCMeta):
    ...
    def _e_step(self, X):
        ...
        """
        log_prob_norm, log_resp,_ = self._estimate_log_prob_resp(X)
    ...
    def predict_proba(self, X):
        ...
        check_is_fitted(self)
        X = self._validate_data(X, reset=False)
        _, log_resp, lpr = self._estimate_log_prob_resp(X)
        return np.exp(log_resp), lpr
    ...
    def _estimate_log_prob_resp(self, X):
        ...
        lpr = weighted_log_prob
        # print("weighted_log_prob", weighted_log_prob.shape)

        log_prob_norm = logsumexp(weighted_log_prob, axis=1)
        with np.errstate(under="ignore"):
            # ignore underflow
            log_resp = weighted_log_prob - log_prob_norm[:, np.newaxis]
        return log_prob_norm, log_resp, lpr
